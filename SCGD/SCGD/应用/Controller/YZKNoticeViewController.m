...
@interface YZKNoticeViewController() <YZKSearchBarViewDelegate, UITableViewDelegate, UITableViewDataSource>
...
@implementation YZKNoticeViewController
...
- (void)setupUI{
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem itemWithTarget:self action:@selector(back) image:@"navigationbar_left-arrow" highImage:nil];
    [self.view addSubview:self.searchView];
    [self.view addSubview:self.tableView];
}

- (void)loadFreshNotice{
    _page = 1;
    [self.dataController requestDataWithType:noticeType params:nil success:^{
...
- (void)loadMoreNotice{
    ...
    [self.dataController requestDataWithType:noticeType params:params success:^{
        if (self.dataController.dataList.count == 0) {
            ...
            _page = _page + 1;
            NSArray *array = self.dataController.dataList;
...
- (UITableView *)tableView{
    if (_tableView == nil) {
        ...
        _tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(loadFreshNotice)];
        _tableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(loadMoreNotice)];
